...
class NumericExpression(object):
    def __init__(self, numeric_expression: Numeric or Predicate):
...
class Function(object):
    """
    A Functions is a factory for Literals.
    ...
    def __init__(self, name: str, arity: int, var_types: [Type] = None, value: NumericExpression = 0,
                 form: str = "="):
        self.name = name
        self.arity = arity
        self.var_types = var_types
    ...
    def __call__(self, *variables):
        return FLiteral(self, list(variables))

    def __str__(self):
        return "{0} {1} {2}".format(self.name, self._form, self._value)
    ...
    def pddl_str(self):
        return "(= ({0} {1}) {2})".format(self, " ".join(self.pddl_variables()), self._value)
...
class Literal:
    ...
    def __init__(self, predicate: [Predicate], variables: [TypedEntity or str]):
...
class FLiteral:
    ...
    def __init__(self, function: Function, variables: [TypedEntity or str]):
        self.function = function
        self.variables = variables

        # Apply types to untyped objects
        if self.function.var_types is not None:
            for i, (expected_type, var) in enumerate(zip(self.function.var_types, self.variables)):
                if not hasattr(var, 'var_type'):
                    # Convert strings
                    self.variables[i] = expected_type(var)

        # Cache str for repr
        # self._str = str(self.function) + '(' + ','.join(map(str, self.variables)) + ')'
        # self._hash = hash(self._str)
    ...
    def __str__(self):
        return str(self.function) + '(' + ','.join(map(str, self.variables)) + ')'

    def __repr__(self):
        return str(self.function) + '(' + ','.join(map(str, self.variables)) + ')'

    def __hash__(self):
        return hash(str(self.function) + '(' + ','.join(map(str, self.variables)) + ')')
    ...
    def pddl_str(self):
        return "({} ({} {}) {})".format(self.function.form, self.function.name, " ".join(self.pddl_variables()),
                                        self.function.value)
...
class LiteralConjunction:
    ...
    def __init__(self, literals: [Literal]):
...
class When:
    """Represents a When over the given variable in the given literal.
    variable is a structs.TypedEntity.
    """
    ...
    def __str__(self):
        return "When ({}) : {}".format(self.variables, self.literal)
...
class Assign:
    """Represents a Assign over the given variable in the given literal.
    variable is a structs.TypedEntity.
    """
    ...
    def __str__(self):
        return "Assign ({}) : {}".format(self.literal, self.variables)
...
### Helpers ###
# Receive a Literal that return the Predicate
